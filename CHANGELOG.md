...
1. **XML 및 SQLX 파일 전용 Ctrl + / 주석 토글 기능 추가**:
   ...
   - 기존의 시스템 내장 주석 커맨드를 가로채는 방식이 이클립스 플랫폼 내부 단축키 결합 컨텍스트에 따라 바인딩이 누락되거나 작동하지 않는 현상을 해결하기 위해, `Alt+Q` 기능과 마찬가지로 자체 **커스텀 주석 커맨드(`sql4ibatis.commands.toggleComment`)**와 단축키(`Ctrl + /`) 매핑을 별도로 새롭게 정의했습니다. 이로써 에디터 고유 단축키 바인딩과 충돌을 우회하며 완벽하게 우선 호출되도록 변경했습니다.
...
### 기능 및 디자인 개선
1. **결과 조회 뷰 아이콘 고도화**:
   - "SQL" 텍스트와 실행 화살표가 직관적으로 드러나도록 가로 배치 및 대폭 확대 적용한 **16x16 픽셀아트 아이콘(`icons/query_result.png`)**을 리뉴얼 제작하여 이식 완료했습니다.
   - 실행 화살표를 세로 최대 높이(12px) 크기로 확대 조정하고, **파란색(좌)에서 주황색(우)으로 부드럽게 흐르는 그라데이션**을 가미(주황색 비중 약 60%)해 시인성을 대폭 끌어올렸습니다.
   - 플러그인 빌드(Export) 시 아이콘 폴더가 최종 패키지에 누락되지 않도록 `build.properties` 및 `plugin.xml` 설정을 동기화 적용했습니다.
2. **뷰 명칭을 'SQL Result'로 전격 변경**:
   - 사용자 직관성 향상을 위해 기존 `Query Result View` 명칭을 일관되게 **`SQL Result`** 로 교체 정의했습니다.
   - `plugin.xml` 내의 뷰 선언부 명칭 및 쿼리 실행 후 동적으로 변경되는 탭 파트명(`setPartName`)을 모두 `SQL Result`로 수정했습니다.
...
2. **워크스페이스 파일 스캔 성능 최적화 (메모리 캐싱 도입)**:
   - 쿼리에 `<include>` 태그가 존재할 때마다 워크스페이스 내 모든 XML/SQLX 파일을 매번 디스크에서 전체 탐색하여 새로 파싱하던 성능 병목 지점을 개선했습니다.
   - 각 파일의 고유 수정 식별자(`modificationStamp`)를 키로 삼아 변경 사항이 발생한 파일만 디바이스에서 로드하고, 변경이 없는 파일은 메모리 캐시(`ConcurrentHashMap`)에서 즉각 반환하는 증분 스캔 알고리즘을 이식했습니다. 이로 인해 최초 스캔 이후의 다음 실행부터는 수 밀리초(ms) 이내의 사실상 실시간에 준하는 속도로 `<include>` 치환이 처리됩니다.
