...
1. **에디터 단축키 실행 (`Alt + Q`)**
   - XML 파일 내 마우스 커서가 위치한 쿼리 블록(`<sql>`, `<select>`, `<insert>`, `<update>`, `<delete>`) 또는 마우스로 블록 지정한 SQL을 즉시 파싱하여 실행합니다.
...
4. **강력한 결과 조회 뷰 (QueryResultView)**
   - **분할 레이아웃 (50:50)**: 상단에는 쿼리 결과 테이블 그리드, 하단에는 실제 파라미터가 바인딩되어 실행된 **완성형 SQL 문장**이 표시됩니다.
